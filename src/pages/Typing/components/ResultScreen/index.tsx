...
import { getAIDailyDictionaries, getChapterLabel, isAIDailyDictionary } from '@/utils/aiDaily'
  ...
  const exportWords = useCallback(() => {
    ...
    import('xlsx')
      .then(({ utils, writeFileXLSX }) => {
        ...
        writeFileXLSX(wb, `${currentDictInfo.name}-${getChapterLabel(currentDictInfo, currentChapter)}.xlsx`)
      })
      .catch(() => {
        ...
        <div className="flex h-screen items-center justify-center">
          <div className="my-card fixed flex w-[90vw] max-w-6xl flex-col overflow-hidden rounded-3xl bg-white pb-14 pl-10 pr-5 pt-10 shadow-lg dark:bg-gray-800 md:w-4/5 lg:w-3/5">
            <div className="text-center font-sans text-xl font-normal text-gray-900 dark:text-gray-400 md:text-2xl">
              {`${currentDictInfo.name} ${isReviewMode ? '错题复习' : getChapterLabel(currentDictInfo, currentChapter)}`}
            </div>
