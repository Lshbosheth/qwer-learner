...
export default function DictDetail({
  ...
  const chapter = useMemo(
    () => (activeChapterIndex != null ? activeChapterIndex : dict.id === currentDictId ? currentChapter : 0),
    [activeChapterIndex, currentChapter, currentDictId, dict.id],
  )
              ...
              <div className="flex w-full flex-wrap gap-3">
                {range(0, dict.chapterCount, 1).map((index) => (
                  <Chapter
                    key={`${dict.id}-${index}`}
                    index={index}
                    chapterIndex={chapterDictIDs ? 0 : undefined}
                    checked={chapter === index}
                    dictID={chapterDictIDs?.[index] ?? dict.id}
                    label={chapterLabels?.[index] ?? dict.chapterLabels?.[index]}
                    onChange={onChapterChange ?? onChangeChapter}
                  />
                ))}
              </div>
            </ScrollArea>
